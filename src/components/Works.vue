<template>
  <section class="works" id="portfolio">
    <div class="works__header" v-scroll-reveal>
      <h2 class="title-primary">RECENT WORKS</h2>

      <h3 class="subtitle-primary">
        It has survived not only five centuries, but also the leap scrambled it
        to make a type.
      </h3>
    ...
    <div
      class="row justify-content-center works__categories category"
      v-scroll-reveal
    >
      <div class="category__item-reset">
        <button
          @click="resetCat"
          class="category__item-name"
          :class="{ noCategorySelected: noCategorySelected }"
        >
          All
        </button>
      </div>
      <div class="category__item" v-for="cat in categories">
    ...
    <div class="works__items" v-scroll-reveal>
      <div class="row justify-content-center">
        <div
          class="col-12 col-sm-6 col-lg-4 col-xl-3 works__item"
          v-for="item in works"
        >
  ...
  data() {
    return {
      slickOptions: {
        ...
        draggable: false,
        prevArrow:
          '<button class="slider__arrow slider__arrow--left test-class"><i class="fas fa-angle-left"></i></button>',
        nextArrow:
          '<button class="slider__arrow slider__arrow--right"><i class="fas fa-angle-right"></i></button>',
        responsive: [
          {
            breakpoint: 576,
            settings: {
